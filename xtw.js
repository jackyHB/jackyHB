...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.xtwqd : $.getdata('xtwqd')) || '';
...
!(async () => {
	if (typeof $request !== "undefined") {  // 严格不相等
		...
		let ckArr = await Variable_Check(ckStr, "xtwqd");
		// await tips(ckArr);
...
/**
 * 签到信息    httpPost
 * https://www.xtwtour.com/api/activity/normal/sign_in/create
 */
function sign_info(timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url: `https://www.xtwtour.com/api/activity/normal/sign_in/create`,
            headers: {
            'Accept-Encoding' : `gzip,compress,br,deflate`,
            'content-type' : `application/json;charset=UTF-8`,
            'Connection' : `keep-alive`,
            'Referer' : `https://servicewechat.com/wx951d94414955eec1/73/page-frame.html`,
            'Host' : `www.xtwtour.com`,
            'User-Agent' : `Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.17(0x1800112e) NetType/4G Language/zh_CN`,
...
// console.log(data);
                if (result.code == 0) {
                  DoubleLog(`\n${result.msg} 总共：${result.totalPoints}积分`)
                    // await $.wait(3 * 1000)
                } else {
                  DoubleLog(`\n${result.msg}`)
                }
